...
namespace Loteria.Classes
{
    internal class Loterias
    {
        ...
        public async Task<T> GetPreviousResultAsync<T>(string loteria, int concurso)
        {
            try
            {
                var response = await client.GetAsync($"https://loteriascaixa-api.herokuapp.com/api/{loteria}/{concurso}");
